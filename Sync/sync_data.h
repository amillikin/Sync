...
public:
	...
	struct ascii_map {
		...
		*/
		static std::unordered_map<int, std::string> createTransMap() {
		...
		static const std::unordered_map<int, std::string> ascii_trans;
		
		/*
		Constant Map for ASCII containing characters representing binary number WITH odd parity
		Used during receiving transmission. Failure to find a value will return TRANSMISSION ERROR.
		*/
		static std::unordered_map<std::string, int> createRecMap() {
	...
	// Struct to hold the SYN and LENGTH chars
	struct frameHeader
	{
		char synChars[16];
		char lenChar[8];
	...
	//Returns FileSize
	static int getFileSize(std::ifstream& inFile);
	...
	*/
	static std::string transError(std::string s, int errChance);
};